...
public partial class BattleSimulation {
 ...
 public bool CanMerge(Building from,Building into)=>result==-2&&from!=null&&into!=null&&from!=into&&from.alive&&into.alive&&from.kind!="base"&&from.kind==into.kind&&from.team==into.team&&from.field==into.field&&from.tier==into.tier&&from.tier<4&&from.built<=time&&into.built<=time&&(from.kind!="mine"||from.workers+into.workers<=6);
 public bool CanMergeCard(int team,string kind,Building into)=>result==-2&&into!=null&&into.alive&&into.team==team&&into.kind==kind&&into.tier==1&&into.built<=time&&!catalog.IsSpell(kind)&&kind!="base";
 ...
 public void Tick(float dt){if(result!=-2)return;time+=dt;effects.RemoveAll(e=>time-e.time>.35f);
  ...
  foreach(var f in fighters.ToArray()){if(!f.alive)continue;if(f.shieldUntil<=time)f.shield=0;if(f.poisonUntil>=time&&f.poisonTick<=time){hits.Add(new DamageHit{target=f,damage=f.poisonDamage,team=f.poisonTeam,sourceId=f.poisonSource});f.poisonTick+=1;}Support(f);
   ...
   if(distance<=f.stats.range&&sight){f.path.Clear();if(time>=f.cooldown){Attack(f,target);float rate=AttackInterval(f);f.cooldown=time+rate;f.flash=time+.12f;} }
   else{if(time>=f.nextPath||target.id!=f.target){f.path=Path(f,target);f.nextPath=time+.6f;}if(f.path.Count>0){Vector2 next=f.path[0];if(!Navigable(f.field,next)){f.path.Clear();f.nextPath=0;}else{Vector2 move=Vector2.MoveTowards(f.pos,next,f.stats.speed*dt);if(SegmentClear(f.field,f.pos,move)){f.walked+=Vector2.Distance(move,f.pos);f.pos=move;f.stuckTime=0;}else{f.stuckTime+=dt;if(f.stuckTime>.2f){f.path.Clear();f.nextPath=0;}}if(f.path.Count>0&&Vector2.Distance(f.pos,next)<.0001f){f.pos=next;f.path.RemoveAt(0);}}}}
   f.target=target.id;
  }
  TickProjectiles(dt);
