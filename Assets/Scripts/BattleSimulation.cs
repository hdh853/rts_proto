...
public class Building:Entity {
 public string kind;public int x,y,branch=-1,workers,invested;public float progress,built,attackReady,boostUntil;public Cell[] shape;public UnityEngine.Rect[] body;
 public bool Pending=>tier==4&&branch<0;
}
public class Fighter:Entity {
 public Vector2 previousPos,facing=Vector2.up,pathTarget;public bool moving;public string kind;public TierDefinition stats;public bool hero,boss,stationary;public float cooldown,supportReady,poisonUntil,poisonTick,poisonDamage,hasteUntil,nextPath,walked,chargeReady,flash;
 public int branch,kills,poisonSource=-1;public float stuckTime;public int poisonTeam,target=-1;public List<Vector2> path=new List<Vector2>();public Vector2 home;
}
public class PlayerState {
 ...
 public int heroKills;public int swaps=1,bought,soul;public float swapReady;public bool heroUnlocked;public int heroLevel=1,heroXp;public float reviveAt=-1,skillAt;
}
...
public partial class BattleSimulation {
 ...
 struct DamageHit{public Entity target;public float damage;public int team,sourceId;}
 ...
 public bool CanPlace(string kind,int team,int f,int x,int y,out string reason){
  ...
  foreach(var c in Shape(kind)){int a=x+c.x,b=y+c.y;
   ...
   if(fighters.Any(u=>u.alive&&u.field==f&&Footprint.Contains(Footprint.Body(Shape(kind)),u.pos-new Vector2(x,y),.08f))){reason="유닛이 지나간 뒤 배치하세요";return false;}}
  return true;
 }
 ...
 public Building AddBuilding(string kind,int team,int f,int x,int y,bool paid=true){
  ...
  var shape=Shape(kind);var b=new Building{id=nextId++,kind=kind,team=team,field=f,x=x,y=y,shape=shape,body=Footprint.Body(shape),hp=hp,maxHp=hp,built=paid?time+1:time,invested=paid?Cost(kind):0};
 ...
 public bool PlaceCard(int team,int slot,int field,int x,int y){var p=players[team];if(slot<0||slot>3||p.hand[slot]==null||result!=-2)return false;string k=p.hand[slot];if(p.gold<Cost(k)){message="골드가 부족합니다";return false;}
  ...
  var mergeTarget=At(field,x,y);if(mergeTarget!=null)return MergeCard(team,slot,mergeTarget);
 ...
 public bool CanStartMerge(Building b)=>b!=null&&b.alive&&b.team==0&&b.kind!="base"&&b.kind!="mine"&&b.tier<4&&b.built<=time&&result==-2;
 public bool CanMerge(Building from,Building into)=>result==-2&&from!=null&&into!=null&&from!=into&&from.alive&&into.alive&&from.kind!="base"&&from.kind!="mine"&&from.kind==into.kind&&from.team==into.team&&from.field==into.field&&from.tier==into.tier&&from.tier<4&&from.built<=time&&into.built<=time&&(from.kind!="mine"||from.workers+into.workers<=6);
 public bool CanMergeCard(int team,string kind,Building into)=>result==-2&&into!=null&&into.alive&&into.team==team&&into.kind==kind&&into.tier==1&&into.built<=time&&!catalog.IsSpell(kind)&&kind!="base"&&kind!="mine";
 public bool MergeCard(int team,int slot,Building into){var p=players[team];if(slot<0||slot>=4||!CanMergeCard(team,p.hand[slot],into)||p.gold<Cost(p.hand[slot]))return false;string kind=p.hand[slot];float hp=catalog.Find(kind)?.buildingHp??catalog.Card(kind).hp;var material=new Building{kind=kind,team=team,field=into.field,tier=1,hp=hp,maxHp=hp,invested=Cost(kind),built=time,alive=true};if(!Merge(material,into))return false;p.gold-=Cost(kind);p.hand[slot]=null;p.refill[slot]=time+.8f;message="카드 직접 합성 · T2";return true;}
 public bool Merge(Building from,Building into){if(!CanMerge(from,into)){message="동일 건물·티어와 일꾼 상한을 확인하세요";return false;}
 ...
 public int WorkerCost(Building mine)=>mine==null||mine.kind!="mine"||mine.workers>=6?0:catalog.workerCosts[Mathf.Clamp(mine.workers,0,5)];
 public bool BuyWorker(Building b){if(b==null||!b.alive||b.kind!="mine"||b.built>time||b.workers>=6)return false;var p=players[b.team];int c=WorkerCost(b);if(p.gold<c){message="골드가 부족합니다";return false;}p.gold-=c;p.bought++;b.workers++;message="일꾼 추가";return true;}
 ...
 public Fighter Spawn(UnitDefinition def,int tier,int branch,int team,int field,Vector2 pos){var t=def.Tier(tier,branch);float m=team==0&&progress!=null?1+.03f*(progress.Level(def.id)-1):1;var u=new Fighter{id=nextId++,kind=def.id,team=team,field=field,pos=pos,previousPos=pos,facing=team==0?Vector2.up:Vector2.down,home=pos,tier=tier,branch=branch,stats=t,maxHp=Mathf.Round(t.hp*m),hp=Mathf.Round(t.hp*m),cooldown=time+t.attackInterval*.35f,supportReady=time+6};fighters.Add(u);return u;}
 ...
 void TryRevive(int team){var p=players[team];if(result!=-2||!p.heroUnlocked||Hero(team)!=null||p.reviveAt<0||time<p.reviveAt||Count(team)>=60)return;var b=Base(team);if(b==null||!TryExit(b,out var pos))return;
  float mul=1+.12f*(p.heroLevel-1);var t=new TierDefinition{name="국경대장",hp=900*mul,damage=65*mul,attackInterval=1.2f,range=.75f,speed=.95f,effect="hero",targetLimit=1};var h=new Fighter{id=nextId++,team=team,field=0,hero=true,kind="hero",pos=pos,previousPos=pos,facing=team==0?Vector2.up:Vector2.down,home=pos,hp=t.hp,maxHp=t.hp,stats=t,kills=p.heroKills,cooldown=time+.42f};fighters.Add(h);p.reviveAt=-1;message="영웅 출전 · Lv"+p.heroLevel;}
 ...
 public float Distance(Vector2 p,Entity e){var b=e as Building;return b==null?Mathf.Max(0,Vector2.Distance(p,e.pos)-.12f):Footprint.Distance(b.body??Footprint.Body(b.shape),p-new Vector2(b.x,b.y));}
 public float EffectiveDamage(Fighter f){float value=f.stats.damage;if(f.team==0&&progress!=null&&catalog.Find(f.kind)!=null)value*=1+.03f*(progress.Level(f.kind)-1);return value;}
 public float AttackInterval(Fighter f)=>f.stats.attackInterval*(f.hasteUntil>time?.85f:1)*(f.stats.effect=="berserk"&&f.hp<=f.maxHp*.5f?.75f:1);
 public float TowerRange(Building b)=>catalog.IsTower(b.kind)?catalog.Card(b.kind).range+(b.tier==4&&b.branch==1?1:0):0;
 ...
 Entity Target(Fighter f){float radius=Mathf.Max(f.stats.detectionRange,f.stats.range+1);var candidates=Enemies(f).Where(e=>Distance(f.pos,e)<radius).ToList();if(candidates.Count==0){if(f.stationary||f.team>1)return null;if(f.field==1)return Enemies(f).OrderBy(e=>Vector2.Distance(f.pos,e.pos)).ThenBy(e=>e.id).FirstOrDefault();return Base(1-f.team);}
 ...
 void Hit(Entity source,Entity target,float damage){hits.Add(new DamageHit{target=target,damage=damage,team=source.team,sourceId=source.id});effects.Add(new AttackEvent{from=source.pos,to=target.pos,field=source.field,time=time,spell=source is Fighter f&&f.stats.range>1});}
 void Attack(Fighter f,Entity target){float dmg=f.stats.damage;var def=catalog.Find(f.kind);if(f.team==0&&progress!=null&&def!=null)dmg*=1+.03f*(progress.Level(f.kind)-1);
  ...
  if(f.stats.effect=="poison"&&target is Fighter poison){poison.poisonUntil=time+3;poison.poisonDamage=Mathf.Max(poison.poisonDamage,dmg*f.stats.effectPower);poison.poisonTeam=f.team;poison.poisonSource=f.id;if(poison.poisonTick<=time)poison.poisonTick=time+1;}
 }
 ...
 public void Tick(float dt){if(result!=-2)return;time+=dt;effects.RemoveAll(e=>time-e.time>.35f);
  ...
  foreach(var b in buildings.ToArray()){if(!b.alive||b.built>time)continue;if(catalog.IsTower(b.kind)){if(time>=b.attackReady){var card=catalog.Card(b.kind);float range=TowerRange(b);var target=Enemies(b).Where(e=>Distance(b.pos,e)<=range&&Sight(b.field,b.pos,e.pos)).OrderBy(e=>Distance(b.pos,e)).FirstOrDefault();if(target!=null){Launch(b,target,card.damage*Mathf.Pow(1.6f,b.tier-1),new TierDefinition{range=range,projectile=b.kind=="crossbow"?"pierce":b.kind=="magicTower"?"blast":"line",projectileSpeed=b.kind=="magicTower"?5:8,pierce=b.kind=="crossbow"?2:1,splash=card.radius,targetLimit=6});b.attackReady=time+card.interval*(b.tier==4&&b.branch==0?.7f:1);}}continue;}
  ...
  foreach(var f in fighters.ToArray()){if(!f.alive)continue;f.previousPos=f.pos;f.moving=false;if(f.shieldUntil<=time)f.shield=0;if(f.poisonUntil>=time&&f.poisonTick<=time){hits.Add(new DamageHit{target=f,damage=f.poisonDamage,team=f.poisonTeam,sourceId=f.poisonSource});f.poisonTick+=1;}Support(f);
   ...
   if(distance<=f.stats.range&&sight){if((target.pos-f.pos).sqrMagnitude>.001f)f.facing=(target.pos-f.pos).normalized;f.path.Clear();if(time>=f.cooldown){Attack(f,target);float rate=AttackInterval(f);f.cooldown=time+rate;f.flash=time+.12f;} }
   else{bool changed=target.id!=f.target;bool refresh=time>=f.nextPath&&(f.path.Count==0||target is Fighter&&Vector2.Distance(target.pos,f.pathTarget)>.5f);if(changed||refresh){f.path=Path(f,target);f.pathTarget=target.pos;f.nextPath=time+.6f;}Advance(f,dt);}

   f.target=target.id;
  }
  TickProjectiles(dt);
  var killed=new Dictionary<int,int>();foreach(var hit in hits){if(!hit.target.alive||hit.target.hp<=0)continue;float damage=hit.damage;bool guarded=fighters.Any(g=>g.alive&&g.team==hit.target.team&&g.field==hit.target.field&&g.id!=hit.target.id&&g.stats.effect=="guard"&&Vector2.Distance(g.pos,hit.target.pos)<1.2f);guarded|=buildings.Any(b=>b.alive&&b.kind=="fence"&&b.tier==4&&b.branch==1&&b.team==hit.target.team&&b.field==hit.target.field&&Vector2.Distance(b.pos,hit.target.pos)<1.5f);if(guarded)damage*=.9f;float shield=Mathf.Min(hit.target.shield,damage);hit.target.shield-=shield;hit.target.hp-=damage-shield;if(hit.target.hp<=0){killed[hit.target.id]=hit.team;if(hit.target is Fighter&&hit.team!=hit.target.team){var owner=fighters.FirstOrDefault(u=>u.id==hit.sourceId);if(owner!=null){owner.kills++;if(owner.hero){players[owner.team].heroKills++;var current=Hero(owner.team);if(current!=null)current.kills=players[owner.team].heroKills;}}}}}hits.Clear();
  ...
  bool ours=Base(0)!=null,theirs=Base(1)!=null;if(!ours&&!theirs)result=2;else if(!ours)result=1;else if(!theirs)result=0;else if(time>=catalog.matchSeconds)result=1;
  if(enableAI&&result==-2&&time>=nextAI){string previousMessage=message;AI();message=previousMessage;nextAI=time+(map==0?3:2);}
  if(fighters.Count>200){var retained=new HashSet<int>(projectiles.Select(p=>p.source.id).Concat(fighters.Where(u=>u.alive&&u.poisonUntil>=time).Select(u=>u.poisonSource)));fighters.RemoveAll(f=>!f.alive&&!retained.Contains(f.id));}if(buildings.Count>150)buildings.RemoveAll(b=>!b.alive);
 }
