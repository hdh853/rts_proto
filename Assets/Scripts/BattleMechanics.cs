...
public class PlacementGesture {
 ...
 public bool Release(BattleSimulation sim,bool inside,int cx,int cy){Move(inside,cx,cy);bool commit=active&&!cancelled&&inside;active=false;return commit&&sim.PlaceCard(0,slot,field,x,y);}
...
public partial class BattleSimulation {
 ...
 public bool Navigable(int field,Vector2 p){
  ...
  foreach(var b in buildings){if(!b.alive||b.field!=field)continue;foreach(var c in b.shape)
   if(p.x>b.x+c.x+.22f-radius&&p.x<b.x+c.x+.78f+radius&&p.y>b.y+c.y+.22f-radius&&p.y<b.y+c.y+.78f+radius)return false;}
  return true;
 }
 public bool SegmentClear(int field,Vector2 a,Vector2 b){int n=Mathf.Max(1,Mathf.CeilToInt(Vector2.Distance(a,b)/.1f));for(int i=1;i<=n;i++)if(!Navigable(field,Vector2.Lerp(a,b,(float)i/n)))return false;return true;}
 ...
 List<Vector2> Path(Fighter f,Entity target){
  ...
  var q=new Queue<int>();var prev=Enumerable.Repeat(-2,w*h).ToArray();q.Enqueue(start);prev[start]=-1;int end=-1;
  ...
  var path=new List<Vector2>();if(end<0)return path;for(int k=end;k!=start&&k>=0;k=prev[k])path.Add(new Vector2((k%w+.5f)/4,(k/w+.5f)/4));path.Reverse();
  if(path.Count==0&&end==start&&Distance(f.pos,target)>f.stats.range)path.Add(new Vector2((start%w+.5f)/4,(start/w+.5f)/4));return path;
 }
 public bool CanUse(int team,int slot,int f,int x,int y,out string reason){reason="";if(slot<0||slot>=4||players[team].hand[slot]==null){reason="카드를 선택하세요";return false;}string id=players[team].hand[slot];if(players[team].gold<Cost(id)){reason="골드가 부족합니다";return false;}return catalog.IsSpell(id)?CanCast(id,team,f,new Vector2(x+.5f,y+.5f),out reason):CanPlace(id,team,f,x,y,out reason);}
 public bool CanCast(string id,int team,int field,Vector2 pos,out string reason){
  reason="";if(!Inside(field,(int)pos.x,(int)pos.y)||pos.x<0||pos.y<0||(field==1&&(map!=2||dungeonCleared))){reason="필드 안을 선택하세요";return false;}
  var c=catalog.Card(id);if(c==null||c.category!="spell"){reason="마법이 아닙니다";return false;}
  bool valid=c.behavior=="mining"?buildings.Any(b=>b.alive&&b.team==team&&b.field==field&&b.kind=="mine"&&b.built<=time&&(int)b.pos.x==(int)pos.x&&(int)b.pos.y==(int)pos.y):c.behavior=="rally"?fighters.Any(u=>u.alive&&u.team==team&&u.field==field&&Vector2.Distance(u.pos,pos)<=c.radius):buildings.Cast<Entity>().Concat(fighters).Any(e=>e.alive&&e.team!=team&&e.field==field&&Distance(pos,e)<=c.radius);
  if(!valid)reason="범위 안에 효과를 받을 대상이 없습니다";return valid;
 }
 public bool CastCard(int team,int slot,int field,Vector2 pos){
  ...
  if(c.behavior=="mining"){var b=buildings.First(e=>e.alive&&e.team==team&&e.field==field&&e.kind=="mine"&&(int)e.pos.x==(int)pos.x&&(int)e.pos.y==(int)pos.y);b.boostUntil=time+c.duration;}
  else if(c.behavior=="rally"){foreach(var u in fighters.Where(e=>e.alive&&e.team==team&&e.field==field&&Vector2.Distance(pos,e.pos)<=c.radius))u.hasteUntil=time+c.duration;}
  else {foreach(var e in buildings.Cast<Entity>().Concat(fighters).Where(e=>e.alive&&e.team!=team&&e.field==field&&Distance(pos,e)<=c.radius))hits.Add(new DamageHit{target=e,team=team,damage=c.damage});}
 ...
 void ProjectileHit(Projectile p,Entity e,float damage){hits.Add(new DamageHit{target=e,damage=damage,team=p.team});effects.Add(new AttackEvent{from=p.pos,to=e.pos,field=p.field,time=time,spell=true});
  if(p.poison>0&&e is Fighter f){f.poisonUntil=time+3;f.poisonTeam=p.team;f.poisonDamage=Mathf.Max(f.poisonDamage,damage*p.poison);f.poisonTick=time+1;}}
