...
public partial class BattleSimulation {
 ...
 List<Vector2> Path(Fighter f,Entity target){
  ...
  var path=new List<Vector2>();if(end<0)return path;for(int k=end;k>=0;k=prev[k])path.Add(new Vector2((k%w+.5f)/4,(k/w+.5f)/4));path.Reverse();if(path.Count>0&&Vector2.Distance(f.pos,path[0])<.0001f)path.RemoveAt(0);return SmoothPath(f.field,f.pos,path);
 }

 List<Vector2> SmoothPath(int field,Vector2 start,List<Vector2> raw){var result=new List<Vector2>();int index=0;while(index<raw.Count){int next=raw.Count-1;while(next>index&&!SegmentClear(field,start,raw[next]))next--;if(!SegmentClear(field,start,raw[next]))return new List<Vector2>();result.Add(raw[next]);start=raw[next];index=next+1;}return result;}
 void Advance(Fighter f,float dt){float budget=Mathf.Max(0,f.stats.speed*dt);int guard=f.path.Count+1;while(budget>.00001f&&f.path.Count>0&&guard-->0){var next=f.path[0];float distance=Vector2.Distance(f.pos,next);if(distance<.00001f){f.path.RemoveAt(0);continue;}float step=Mathf.Min(budget,distance);var move=Vector2.MoveTowards(f.pos,next,step);if(!Navigable(f.field,next)||!SegmentClear(f.field,f.pos,move)){f.path.Clear();f.nextPath=0;break;}var delta=move-f.pos;f.pos=move;f.walked+=step;f.moving=true;f.facing=delta.normalized;budget-=step;if(distance<=step+.00001f)f.path.RemoveAt(0);}}
