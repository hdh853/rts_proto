...
이전 v0.4: 티어별 자체 3D 모형·합성 후보 강조·유닛 정보·원형 마법·직접 합성 및 이동 오류 수정. Windows·Android 빌드 및 Unity 검사 69개 통과. 자동 화면 확인 완료, 실기기 테스트와 아트 가독성 보완 대기.

- [v0.4 명세](docs/specs/004-visual-tiers-selection-navigation.md)
- [v0.4 실행 및 테스트](docs/testing-v04.md)
- [v0.4 검증 기록](docs/verification-v04.md)

Unity 6000.4.0f1 기반 세로형 RTS + 배낭 합성 프로토타입. Android 우선이며 Windows 빌드로도 검증한다.

v0.3 기반 기능: 종족 생산6종과 공용 기능·마법8종에서 8장 편성. 중복 없는 4장 손패, 반투명 배치 미리보기와 손을 뗄 때 결제, 1칸 금광·일꾼6명, T4 합성, 넓어진 전장·건물 사이 경로, 지연 피해 투사체, 고정 적 던전·영웅·영구 성장을 포함한다.

- [이전 변경 명세 v0.3](docs/specs/003-placement-decks-projectiles.md)
...
이전 v0.3 Windows·Android 빌드와 Unity 규칙 검사54개는 성공했다. 메인·덱·초록/빨강 배치·전장·던전·T4 화면을 확인했다. 실기기 터치·성능 검증은 남아 있다. 초기 수치는 튜닝 가능한 가설이며, 종족 밸런스가 검증 완료된 것은 아니다. 아트는 직접 생성한 임시 카툰 모형이다.
